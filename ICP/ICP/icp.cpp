    ...
    mclOutputHandlerFcn print_handler)
{
    ...
    if (!mclmcrInitialize())
        return false;
    if (!GetModuleFileName(GetModuleHandle("icp"), path_to_dll, _MAX_PATH))
        return false;
...
{
    return icpInitializeWithHandlers(mclDefaultErrorHandler, mclDefaultPrintHandler);
}
...
{
    return mclFeval(_mcr_inst, "icp", nlhs, plhs, nrhs, prhs);
}

LIB_icp_CPP_API 
void MW_CALL_CONV icp(const mwArray& name1, const mwArray& name2)
{
    mclcppMlfFeval(_mcr_inst, "icp", 0, 0, 2, &name1, &name2);
}
